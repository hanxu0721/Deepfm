...
flags.DEFINE_integer("batch_norm", 1, "batch_norm")
flags.DEFINE_float('batch_norm_decay',0.995, 'batch_norm_decay')
#flags.DEFINE_boolean("train_phase", True, "batch_norm")
...
def train(Xi, Xv, y, local_step=None, num_examples=None, startt=None):
    ...
    if local_step % 10 == 0 and FLAGS.task_index == 0:
        num_examples += len(y)
        _, cost_value,loss_value, auc_value, step = sess.run([train_op, cost,loss, auc, global_step], feed_dict)
...
if FLAGS.job_name == "ps":
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        cost,loss,auc = model.loss(sparse_id,sparse_value,label,dropout_keep_deep,train_phase) 

...
num_ins=10000000
num_runs=1
feature_size=8230571


# 超参
info=" max_epoch=${max_epochs} batch_size=${batch_size} factor_size=${factor_size}"
...
train(){
    ...
    --files=./DataParser.py,./deepfm.py,./train_lr.py,./deep_input.py \
    ...
    --log_dir=${log_dir}/  \
    --train_dir=${model}/ \
    --command=Python/bin/python train_lr.py num_runs=${num_runs} num_ins=${num_ins} field_size=${field_size} feature_size=${feature_size} ${info} ${learning} ${reg}
}
train

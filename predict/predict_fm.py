...
filenames.sort(key=lambda x : x.split('/')[-1].split('_')[0])
...
with tf.Session() as sess:
    # 导入模型
    saver = tf.train.import_meta_graph("/data4/ads_dm/hanxu8/DeepFm/model/model.ckpt-111860.meta", clear_devices=True)
    saver.restore(sess, '/data4/ads_dm/hanxu8/DeepFm/model/model.ckpt-111860')
    ...
    for i in ins_list:
        Xi, Xv, y = decode_test(ins=i,field_size=field_size)
    #print (Xi)
        ...
        feed_dict={sparse_id:Xi,
               sparse_value:Xv,
               dropout_keep_deep:drop_out
               }
        pred_value=  sess.run(pred,feed_dict = feed_dict)
    #print(pred_value)
       
        #with open('/data4/ads_dm/hanxu8/DeepFm/predict/result/{}'.format(i.split('/')[-1]),'w') as f:
            #for i in range(len(pred_value)-1):
                #f.write(str(pred_value[i][0])+'\n')
            #f.write(str(pred_value[len(pred_value)-1][0]))
        
        for i in range(len(pred_value)):
            print (str(y[i])+','+str(pred_value[i][0]))
